...
from DB.database import ALLOWED_TYPES

def _coerce_int(x):
    try: return int(x)
    except: return x  # fallback
...
def render():
    ...
    if submitted:
        ...
        if init_bal_str.strip():
            ...
            except: st.error("Initial Balance must be a number."); return
    ...
    df = list_bank_accounts()
    if df.empty:
        st.info("Nothing to edit yet."); return

    # by_title = {f"{r['title']} (#{r['id']})": int(r["id"]) for _, r in df.iterrows()}
    by_title = {f"{r['title']} (#{r['id']})": _coerce_int(r["id"]) for _, r in df.iterrows()}
